...
- The board has `dir="rtl"`, so the default flex row already lays children right-to-left. JS appends only the actual characters in logical order (char[0] → visually rightmost) — no padding cells. Each row sizes to its own char count, and the board's `align-items: center` (flex column) centres shorter rows within the longest-row width. **Do not add `flex-direction: row-reverse`** — that double-reverses the layout and Hebrew reads backwards.
- `buildGrid` sets `--cells-across` on `.board` to the actual column count (1–13). The CSS clamp on `--cell-w` divides by that var, so short messages don't blow past the 96 px ceiling and long ones shrink proportionally on narrow viewports.
