...
- The board has `dir="rtl"`, so the default flex row already lays children right-to-left. JS appends characters in logical order (char[0] → rightmost), then trailing padding cells. **Do not add `flex-direction: row-reverse`** — that double-reverses the layout and Hebrew reads backwards.
