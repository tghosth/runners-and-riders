...
- The board has `dir="rtl"`, so the default flex row already lays children right-to-left. JS appends `padBefore` padding cells first (visually rightmost), then characters in logical order (char[0] → rightmost char), then `padAfter` padding cells (visually leftmost). This keeps each row visually centered. **Do not add `flex-direction: row-reverse`** — that double-reverses the layout and Hebrew reads backwards.
