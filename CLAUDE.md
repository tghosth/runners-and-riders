...
- The first board row is the **clock + Hebrew date header** — built from the same flap cells as the body. Layout: `DATE_COLS = 8` cells of Hebrew date on the visual right (`[day-gematria] [month]`, year omitted to fit), `TIME_COLS = 5` cells of `HH:MM` Asia/Jerusalem time on the visual left, and any leftover cells become padding in the middle. `dayGematria()` builds the Hebrew day numerals manually (with the conventional ט״ו / ט״ז for 15 / 16) since `nu-hebr` rendering is uneven across browsers. `formatHebrewDate()` strips geresh / gershayim if the date overflows 8 cells (typical leap-year Adar A / B).
- `buildGrid` always pads body rows to `MAX_COLS = 13` so every row — including the header — has the same flap count and cells stay uniform width across rows.
